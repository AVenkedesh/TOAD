# -*- coding: utf-8 -*-
"""
Created on Tue Oct 15 13:31:35 2024
...
for i in range(conf_mat.shape[0]):
    ...
    FP = np.sum(conf_mat[:, i]) - TP  # False Positives
    FN = np.sum(conf_mat[i, :]) - TP  # False Negatives
    
    # Calculate Dice coefficient
    dice_coefficient = (2 * TP) / (2 * TP + FP + FN) if (TP + FP + FN) > 0 else 0
    dice_coefficients.append(dice_coefficient)
    
# Print the results
for i, dice in enumerate(dice_coefficients):
    print(f'Dice Coefficient for Class {i}: {dice:.4f}')